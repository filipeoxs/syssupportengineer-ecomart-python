...
class TestPricePlanService(TestCase):
    ...
    def tearDown(self):
        # Clean up the price plan repository after each test
        price_plan_repository.clear()

    def test_calculate_costs_against_all_price_plans(self):
        # Test calculating costs against all price plans
        # Store some price plans in the repository
        ...
        # Mock the retrieve_readings_for method of electricity_reading_service
        ...
        # Calculate spend against each price plan for a smart meter
        spend = self.price_plan_service.get_list_of_spend_against_each_price_plan_for('smart-meter-1001')
        
        # Assert the spend against each price plan matches the expected values
