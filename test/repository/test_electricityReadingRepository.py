...
class TestElectricityReadingRepository(TestCase):
    def setUp(self):
        # Set up the ElectricityReadingRepository and add initial readings
    ...
    def test_have_new_entry_when_new_smart_meter_id_is_given(self):
        # Test adding new entry when a new smart meter ID is given
        readings = self.electricity_reading_repository.find('smart-meter-0')
        # Assert the stored readings match the expected values
    ...
    def test_add_usage_data_against_smart_meter_id_if_it_already_exists(self):
        # Test adding usage data against smart meter ID if it already exists
        self.electricity_reading_repository \
            .store(
                'smart-meter-0',
                [ElectricityReading({'time': 1510572000, 'reading': 0.32})])
        readings = self.electricity_reading_repository.find('smart-meter-0')
        # Assert the number of readings after adding new data
        self.assertEqual(3, len(readings))
        # Assert the new reading is included in the stored readings
